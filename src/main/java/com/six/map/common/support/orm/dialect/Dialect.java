package com.six.map.common.support.orm.dialect;
...
public abstract class Dialect {
    ...
    public String getCountString(String sql) {
        ...
        if (select.toLowerCase().indexOf("select distinct") != -1 || sql.toLowerCase().indexOf("group by") != -1) {
            return new StringBuffer(sql.length()).append("select count(1) count from (").append(sql, 0, orderIndex).append(" ) t").toString();
        } else {
            return new StringBuffer(sql.length()).append("select count(1) count ").append(sql, formIndex, orderIndex).toString();
        }
    }
